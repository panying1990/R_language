...
# 数据标准化处理==============================
target_phone<-data.frame(tellphone =rep(1,nrow(weixin_data)),taobaoid =rep(1,nrow(weixin_data)),tag =rep(1,nrow(weixin_data),event=rep(1,nrow(weixin_data)))
...
for(i in 1:nrow(weixin_data)){
  if((is.null(weixin_data$淘宝id[i])&is.null(weixin_data$手机或其他信息[i]))==FALSE){
    if(nchar(weixin_data$手机或其他信息[i])==11){
      ...
      target_phone$event[i]<-weixin_data$活动[i]
    }else{
